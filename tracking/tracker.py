...
from .config import (
    ...
    STEER_KP_STRAIGHT, STEER_KP_CURVE,
    STEER_KD_STRAIGHT, STEER_KD_CURVE,
    STEER_PID_CURVE_START, STEER_PID_CURVE_END,
    STEER_KP, STEER_KP_LEFT, STEER_KP_RIGHT,
    STEER_KD, STEER_FF_GAIN, CURVATURE_SMOOTHING_FACTOR,
    STEER_DEAD_ZONE, STEER_OUTPUT_MAX,
    LOOKAHEAD_PX, LOOKAHEAD_KNEE_SPEED,
    LOOKAHEAD_PX_PER_SPEED, LOOKAHEAD_PX_MAX,
...
class SegmentTracker:
    ...
    def __init__(self, model_dir=None, TPEs=1,
        ...
        self._last_speed = None  # 上一帧指令速度（前瞻自适应用；None=用 LOOKAHEAD_PX）
    ...
    def process_frame(self, frame_bgr, return_aux=False):
        ...
        if pool_ok and result is not None:
            ...
            if seg_map is not None:
                ...
                # 前瞻随速度自适应（膝点式）：低速保持 120px 手感，高速拉远提前入弯
                lookahead = LOOKAHEAD_PX
                if self._last_speed is not None:
                    lookahead = LOOKAHEAD_PX + LOOKAHEAD_PX_PER_SPEED * max(
                        0, self._last_speed - LOOKAHEAD_KNEE_SPEED)
                    lookahead = min(lookahead, LOOKAHEAD_PX_MAX)
    ...
    def offset_to_servo(self, offset, curvature=None, dt=1.0,
        ...
        return_details: True 时返回 dict（舵机 + PID 三项分解，供可视化 HUD），
            False 时返回 int（默认，兼容旧调用）。

        返回（return_details=True）:
            {"servo": 最终PWM, "steer": 相对中位打角(µs),
             "error": 真实横向误差, "error_dz": 死区后反馈误差,
             "p"/"d"/"ff": 三项各自贡献(µs), "kp"/"kd": 当前增益,
             "curve_signal": 直道/弯道插值信号}
        """
        ...
        # 2. 根据直道/弯道信号选择 Kp、Kd
        curve_signal = max(abs(offset), abs(curvature))
        curve_amount = clamp(
            (curve_signal - STEER_PID_CURVE_START) /
            (STEER_PID_CURVE_END - STEER_PID_CURVE_START),
            0.0, 1.0
        )

        kp = STEER_KP_STRAIGHT + (STEER_KP_CURVE - STEER_KP_STRAIGHT) * curve_amount
        kd = STEER_KD_STRAIGHT + (STEER_KD_CURVE - STEER_KD_STRAIGHT) * curve_amount

        # 左右不对称补偿：如果配置了单独 Kp，则覆盖上面的 Kp
        ...
        #    2026-08-16：调用方传真实帧间隔 dt（秒）→ 每秒差分，帧率波动不漂参数
        d_error = (offset - self._prev_steer_error) / max(dt, 0.001)

        # 4. PD + 曲率前馈 + 限幅
        p_term = error * kp
        d_term = d_error * kd
        ff_term = curvature * STEER_FF_GAIN
        steer = int(p_term + d_term + ff_term)
        steer = clamp(steer, -STEER_OUTPUT_MAX, STEER_OUTPUT_MAX)

        self._prev_steer_error = offset

        # 5. 映射到舵机 PWM
        servo = int(clamp(SERVO_MID + steer, SERVO_MIN, SERVO_MAX))
    ...
    def compute_speed(self, offset, curvature=None, base_speed=BASE_SPEED):
        ...
        speed = clamp(base_speed - reduction, 0, base_speed)
        self._last_speed = speed  # 供下一帧前瞻自适应
        return speed
